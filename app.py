...
def fetch_cars_from_api(body_type: str, user_input: str = ""):
  ...
  if clean_input:
    # 1. Try searching by MAKE (e.g., bmw, porsche, maruti, ferrari)
    try:
      res = requests.get(
    ...
    # 2. Try searching by MODEL (e.g., 330i, m3, civic, huracan)
    try:
      res = requests.get(
  ...
  # Fallback brand pool spanning mass-market, luxury, supercar, and regional makers
  brand_pool = {
      "SUV": [
          "toyota",
          "honda",
...
with tab1:
  ...
  if st.button("Predict Best Vehicle Fit", type="primary"):
    ...
    if api_cars:
      ...
      st.warning(
          f"No specific matches found for '{c_brand_model}'. Try searching by"
          " Brand name (e.g. BMW, Lamborghini, Maruti, Ferrari, Porsche)."
      )

    reasons = [
        f"Queries live vehicle records for {c_type} models across global"
        " manufacturer databases.",
        f"Fits within or near your target budget of {sym}{c_budget:,.0f} against"
        f" regional {c_type} price baselines ({sym}{avg_cost:,.0f}).",
...
with tab2:
  ...
  if st.button("Analyze Market Opportunities", type="primary"):
    ...
    success_rate = "88%"
    reasons = [
        f"**High Supply Deficit:** EV Market Days Supply in {s_location} is low"
        f" ({s_data['days_supply']['EV']} days), meaning consumer purchases"
        " outpace dealer stock.",
        "**Price Vacuum:** Regional average EV price is high"
        f" ({s_sym}{avg_ev:,.0f}), leaving a wide opening for an affordable"
        " entry competitor.",
        "**Body Style Dominance:** SUV body styles lead sales with"
        f" {s_data['market_share']['SUV']} share, making a Compact EV SUV the"
        " highest probability entry model.",
    ]
